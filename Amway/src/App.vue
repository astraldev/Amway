<template>
  <div id="app">
    <div class="sticky top-0 w-full z-20 bg-blue-600 border-b flex justify-between p-0 flex-col md:flex-row">
      <main class="flex items-center justify-between space-x-3 p-2 w-full md:w-p-70 lg:w-p-85">
        ...
        <div
          ...
          "
        >
          <a href="#" @click.prevent="changePage('SignIn', 'User')" class="inline-flex space-x-1 flex-nowrap">
          <span
      ...
      <div class="block w-full md:flex md:items-center md:w-p-40" >
        <div :class="'border-t-2 border-blue-300 md:border-0 ' + getMenuClass()" class="md:inline-block">
          <ul class="w-full flex flex-col md:flex-row py-1" v-click-outside="closeMenu">
            ...
            >
              <a
                ...
                @click="
                  ...
                  changePage('Shop');
                "
                class="outline-none flex flex-shrink-0 space-x-4 md:space-x-1 w-full"
              >
                <svg
                  class="w-6 h-6"
                  fill="none"
...
export default {
  ...
  methods: {
    _toggleMenu(){
      if(!this.toggleMenu) setTimeout((()=>{this.toggleMenu = !this.toggleMenu}), 100)
    },
    hideNotification() {
      this.notify = false;
    },
    ...
    changePage(name, other) {
      ...
      if(name == "SignIn" && other){
        if(this.signedIn){this.Page = other}
        console.log(other)
      }
